...
void SPI_LCD_Init(void)
{
   ...
   osDelay(10);               // 屏幕刚完成复位时（包括上电复位），需要等待5ms才能发送指令
...
void LCD_CopyBuffer(uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint16_t *DataBuff)
{
    ...
    // 2. 将 DC 引脚拉高，告诉屏幕“接下来发的是像素数据”
    LCD_DC_Data; 

    // 3. 计算要发送的【总字节数】(RGB565 每个像素占 2 字节)
    uint32_t total_bytes = width * height * 2;
    
    // 强转为 8 位指针，准备按字节发送
    uint8_t *ptr = (uint8_t *)DataBuff;

    // 4. 【分块发送机制】：由于 HAL 库的长度参数是 uint16_t (最大65535)
    // 为了防止 115200 字节直接溢出，我们每次最多发 60000 字节
    while(total_bytes > 0) 
    {
        uint16_t send_size = (total_bytes > 60000) ? 60000 : total_bytes;
        
        // 直接用官方最稳妥的 HAL_SPI_Transmit，绝对不要去切 16 位！
        HAL_SPI_Transmit(&hspi6, ptr, send_size, 1000);
        
        ptr += send_size;          // 指针往后挪
        total_bytes -= send_size;  // 剩余未发字节数减少
    }
}
